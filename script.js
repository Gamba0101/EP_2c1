...
function testSupport(supportedDevices) {
    ...
    if (!isSupported) {
        alert(`Это демо, работающее на ${detectedDevice.client.name}/${detectedDevice.os.name}, ` +
            `плохо поддерживается в данное время, продолжайте аккуратно.`);
    }
}
...
new controls
    ...
    .add([
    new controls.StaticText({ title: 'Параметрические руки' }),
    fpsControl,
    new controls.Toggle({ title: 'Зеркалить', field: 'selfieMode' }),
    ...
    new controls.Slider({
        ...
        discrete: ['Лайт', 'Тяжелая'],
    }),
    new controls.Slider({
        title: 'Мин точность обнаружения',
        field: 'minDetectionConfidence',
        range: [0, 1],
        step: 0.01
    }),
    new controls.Slider({
        title: 'Мин точность отслеживания',
